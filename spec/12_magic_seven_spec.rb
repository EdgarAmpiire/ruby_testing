...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'When the previous number is 10 and random number is 3' do
  ...
  describe '#play' do
    ...
    context 'when the random number is 1001' do
      subject(:game_one_thousand_one) { described_class.new(1001) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_one_thousand_one.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 987,654,321' do
      subject(:game_huge) { described_class.new(987654321) }
      # remove the 'x' before running this test
      it 'will return 7' do
